...
public class GenericDao implements IDatabaseOperate
{
    ...
    @Override
    public synchronized boolean updateContent(String tableName, ContentValues values, String whereClause, String[] whereArgs) {
        boolean flag = false;
        SQLiteDatabase database = null;
        int count = 0;
        try {
    ...
    @Override
    public synchronized Map<String, String> getRowData(String tableName, String selection, String[] selectionArgs) {
        SQLiteDatabase database = null;
        Cursor cursor = null;
    ...
    @Override
    public synchronized List<Map<String, String>> getListData(String tableName, String selection, String[] selectionArgs , String orderBy) {
        SQLiteDatabase database = null;
        Cursor cursor = null;
        ...
        try {
            ...
            cursor = database.query(false, tableName, null, selection, selectionArgs, null,
                    null, orderBy, null); //查询所有记录，所以有重复的数据也要全部检出，所以第一参数置为false.
            int cols_len = cursor.getColumnCount();
            while (cursor.moveToNext()) {
                Map<String, String> map = new HashMap<String, String>();
                for (int i = 0; i < cols_len; i++) {
