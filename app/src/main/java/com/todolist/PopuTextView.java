...
public class PopuTextView extends RelativeLayout {
    ...
    public PopuTextView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        timeType = array.getString(R.styleable.PopuTextView_ptv_TimeViewType);
...
//                mImageView.setImageDrawable(mDrawable);
            ...
            public void onFinish() {
                ...
                if (actionListener != null && !mTextView.getText().toString().equalsIgnoreCase( mDefault ) ) {
                    if( timeType != null && timeType.equalsIgnoreCase("y-m-d") ) {
                        Date date = null;
                        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-mm-dd");
                        try {
                            date = sdf.parse(mTextView.getText().toString());
                        } catch (ParseException e) {
                        }

                        if( date != null )
                            actionListener.doSave( date.getTime() );
                    }
                }
            }
